...
class ProcurementGuardrails:
    def validate_proposal(self,product:Product,supplier:Supplier|None,qty:float,unit_cost:float,allow_missing_supplier=False):
        reasons=[]; warnings=[]; value=max(0.0,qty)*max(0.0,unit_cost); pack=max(1,int(product.pack_size))
        ...
        if supplier is None or not supplier.is_active:
            if allow_missing_supplier: warnings.append('No supplier master/lead-time data is available; recommendation uses the configured company default lead time and cannot be executed until a supplier is assigned.')
            else: reasons.append('No active supplier is available.')
    ...
    def validate_approval(self,proposal:ProcurementProposal,product:Product,supplier:Supplier|None,requested_qty:float):
        result=self.validate_proposal(product,supplier,requested_qty,proposal.unit_cost)
        if proposal.status not in ('PENDING','HOLD','APPROVED'): result.allowed=False; result.reasons.append(f'Proposal is in state {proposal.status}, not PENDING.')
